...
`do_bookdown()` adds default steps related to package checks to the
`"install"`, `"before_deploy"`, `"script"` and `"deploy"` stages.

1.  [`step_install_deps()`](https://docs.ropensci.org/tic/reference/step_install_pkg.md)
    in the `"install"` stage

2.  [`step_session_info()`](https://docs.ropensci.org/tic/reference/step_session_info.md)
    in the `"install"` stage.

3.  [`step_setup_ssh()`](https://docs.ropensci.org/tic/reference/step_setup_ssh.md)
    in the `"before_deploy"` to setup the upcoming deployment (if
    `deploy` is set),

4.  [`step_setup_push_deploy()`](https://docs.ropensci.org/tic/reference/step_setup_push_deploy.md)
    in the `"before_deploy"` stage (if `deploy` is set),

5.  [`step_build_bookdown()`](https://docs.ropensci.org/tic/reference/step_build_bookdown.md)
    in the `"deploy"` stage, forwarding all `...` arguments.

6.  [`step_do_push_deploy()`](https://docs.ropensci.org/tic/reference/step_do_push_deploy.md)
    in the `"deploy"` stage.

By default, the `_book/` directory is deployed to the `gh-pages` branch,
keeping the history.
...
- deploy:
  ...
  `[flag]`  
  If `TRUE`, deployment setup is performed before building the bookdown
