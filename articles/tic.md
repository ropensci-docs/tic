...
[Troubleshooting](https://docs.ropensci.org/tic/articles/advanced.html#troubleshooting)
...
See the [CI Client
Packages](https://docs.ropensci.org/tic/articles/ci-client-packages.md)
...
during
[`use_tic()`](https://docs.ropensci.org/tic/reference/use_tic.md). The
latter will always be present because it will be the main CI config file
for all providers from now on. Usually you do not need to touch the YAML
files anymore. All build customization is done in `tic.R` and applies to
all providers. For more information about the build lifecycle in
general, check the [Build
lifecycle](https://docs.ropensci.org/tic/articles/build-lifecycle.md)
article.
...
[`ci_on_ghactions()`](https://docs.ropensci.org/tic/reference/ci.md) is
used as a condition. Why do we do this? Building the pkgdown site on
multiple CI services has no added benefit and might even cause problems
due to race conditions during deployment.

[`ci_on_ghactions()`](https://docs.ropensci.org/tic/reference/ci.md) can
be replaced by one of its sibling functions like
